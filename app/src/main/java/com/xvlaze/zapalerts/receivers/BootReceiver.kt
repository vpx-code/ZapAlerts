...
class BootReceiver : BroadcastReceiver() {
    override fun onReceive(c: Context, intent: Intent) {
        Log.d(TAG, "Boot receiver on!")
        if (Intent.ACTION_BOOT_COMPLETED == intent.action) {
            Log.d(TAG, "Boot receiver on!")
            Log.d(TAG, "[DAILY]")
            if (Daily.doesSavedDateExist(c)) {
                Log.d(TAG, "Saved date exists")
                if (Daily.hasDatePassed(c)) {
                    Log.d(TAG, "Saved date has passed")
                    MyAlarmManager.scheduleAlarm(Daily, c)
                    Daily.updateSavedDate(c)
                } else {
                    Log.d(TAG, "Saved date has not passed")
                }
                MyAlarmManager.scheduleAlarm(Daily, c)
            } else {
                Log.d(TAG, "Saved date does not exist")
                Daily.updateSavedDate(c)
                MyAlarmManager.scheduleAlarm(Daily, c)
            }
